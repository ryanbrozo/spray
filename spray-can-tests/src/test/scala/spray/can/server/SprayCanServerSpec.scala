...
class SprayCanServerSpec extends Specification with NoTimeConversions {
  ...
  class TestSetup extends org.specs2.specification.Scope {
    ...
    val listener = {
      val commander = TestProbe()
      val config = spray.util.pimpString_(configOverrides).toOption.map(ConfigFactory.parseString)
      commander.send(IO(Http), Http.Bind(bindHandler.ref, hostname, port, settings = config.map(ServerSettings.apply)))
